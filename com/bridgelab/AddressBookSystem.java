...
public class AddressBookSystem<details> {
	 ...
	 * List interface.
	 */
	static ArrayList<Contact> arrayDetails = new ArrayList<Contact>();
	/**
	...
	public void addDetails() {
		/**
		 * create a object an contacts class. object name is info
		 */
...
//        sc.close();
	...
	public void editDetails() {
		...
		/**
		 * using for loop a)i=0=initialize; b)i < arrayDetails.size()=condition; c)i++=
		 * increment by 1
		 *
		 * 1) 1st initialize the value of a variable,then check the condition,then go to
		 * if condiion,in this condition arrayDetails object the element at the
		 * specified position in this list then if they are equals then select the
		 * option choose edit detatils then 1st confirm ur first name to edit details
		 * then put ur previous name what u set this, then select what u want
		 * change,suppose u choose first name, then put it new name what u want then its
		 * edit and after the print all the information with editing new name then again
		 * update.
		 *
		 */
		for (int i = 0; i < arrayDetails.size(); i++) {
			if (arrayDetails.get(i).getFirstName().equals(confirmName)) {
				...
				/**
				 * if you will check the name which you have written earlier,whether it matches
				 * or not , if it matches then it will be edited after that. if you put previous
				 * name is incorrect then show the else statement
				 */
	...
	public void duplicateCheck(String firstName) {
		/**
		 * for loop is used check the condition if condition is true then element at the
		 * specified position in this list in arrayDetails all store in contactName
		 */
		for (int k = 0; k < arrayDetails.size(); k++) {
			String contactName = arrayDetails.get(k).getFirstName();

			/**
			 * if 1st name in address boook is present equal to prent in contactName then
			 * print this person is already present otherwise
			 */
	...
	public void createAddressBook() {
		...
		while (true) {
			...
			/**
			 * if u choose option 1 create new address book then enter the name of address
			 * book
			 */
			switch (choose) {
			case 1:
				...
				/**
				 * here arrayDetails is the 1st object and new_address_book is 2nd but both are
				 * storing all details
				 */
				arrayDetails = new_address_book;
				while (true) {
					/**
					 * choose what u want to do if u choose add details then all details put it like
					 * first name , last name,email,zip,mobile no,city,state,address etc if you
					 * choice 5 then exit in all process
					 */
					System.out.println("Choose what you want to do: ");
					System.out
					...
					/**
					 * if all the details in fill then if u enter or choose 4 then u r exist in
					 * process and start with agin new address book
					 */
					if (choose1 == 4) {
						System.out.println("Exited");
						break;
					...
					/**
					 * using switch case in the switch case we are calling the method
					 */
					switch (choose1) {
					case 1:
						details.addDetails();
			...
			case 2:
				...
				if (hashmap.containsKey(address_name_old)) {
					...
					/**
					 * arrayDetails is 1st object in store all the data now here all the data store
					 * in old address book
					 */
					...
					while (true) {
						...
						/**
						 * switch case is used in this switch case we are calling those method if choose
						 * option 1 add contact then calling this method in switch case
						 */
						switch (choose1) {
						case 1:
							details.addDetails();
	...
	public static void main(String[] args) {
		...
		/**
		 * calling method calling method = object name.method name here object is
		 * details and method is createAddressBook()
		 */
